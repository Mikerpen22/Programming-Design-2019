...
int main(){
    ...
    for(int i = 0; i < totalDay; i++){ //if one of the three conditions is ture, break it and return -1; if not, count the costs in 5 segments 
        if((stockAmount[0] + purchasePerDay[i]) / Rate[0] < productPerDay1[i]){ //the goal of station1 is assigned, so need more materials to fulfill, and if we want to meet the goal, we also need "rate * assignAmount" amount of meterials
            result = -1;
            break;
        }
        stockAmount[0] = stockAmount[0] + purchasePerDay[i] - productPerDay1[i] * Rate[0]; //update the original stock of station0
        if((stockAmount[1] + productPerDay1[i]) / Rate[1] < productPerDay2[i]){ //so as the station1, the station1 product amount has to meet the demand of station2
            result = -1;
            break;
        }
        stockAmount[1] = stockAmount[1] + productPerDay1[i] - productPerDay2[i] * Rate[1];
